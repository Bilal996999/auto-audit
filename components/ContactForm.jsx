  ...
  return (
    <form onSubmit={handleSubmit} className='lg:w-[60%] w-full lg:pl-9 pl-0 relative'>
        <div className='flex sm:flex-nowrap flex-wrap justify-center items-center lg:gap-14 md:gap-6 sm:gap-4 gap-2'>
            <input type="text" required onChange={(e)=> setName(e.target.value)} className="h-14 rounded-2xl border border-[#99999981] indent-4 outline-none flex-1 text-[16px] text-[#000]" placeholder="Enter Name"/>
            <input type="email" required onChange={(e)=> setEmail(e.target.value)} className="h-14 rounded-2xl border border-[#99999981] indent-4 outline-none flex-1 text-[16px] text-[#000]" placeholder="Enter Email Address"/>
        </div>
        <textarea required onChange={(e)=> setMessage(e.target.value)} className="pt-4 required: mt-8 rounded-2xl border border-[#99999981] w-full resize-none indent-4 outline-none flex-1 text-[16px] text-[#000]" rows={10} placeholder='Enter your message...' />
